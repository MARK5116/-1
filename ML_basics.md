...
**训练误差**：基于模型关于训练数据集的平均损失。

**测试误差**：基于模型关于测试数据集的平均误差，反能了学习方法的预测能力。

# 特征提取和特征选择

**特征提取：**特征提取是指将机器学习算法不能识别的原始数据转化为算法可以识别的特征的过程。**将机器学习算法不能识别的文本、字符等特征转化为数值特征。**

**特征选择：**指去掉无关特征，保留相关特征的过程，也可以认为是从所有的特征中选择一个最好的特征子集。特征选择本质上可以认为是降维的过程。
...
**泛化能力**：指模型对未知数据的预测能力。一般通过测试集来预测学习方法的泛化能力。

泛化误差：指模型对未知数据预测的误差。反映了学习方法的泛化能力。
...
留出法直接将数据集分成两个互斥的数据集，一个作为训练集S，一个作为测试集T。用S训练模型，用T来评估测试误差，作为泛化误差的估计。

数据采样方式：分层采样，采用若干次随机划分，重复进行实验评估取平均值作为评估结果。

（2）自助法

在包含m个样本的数据集D中，有放回的随机抽取m个样本（一次一个）得到D’数据集，将D’作为测试集，其中D中的36.8%的数据未出现在D’中。
